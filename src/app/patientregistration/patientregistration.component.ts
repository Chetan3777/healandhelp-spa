...
export class PatientRegistrationComponent implements OnInit {
  ...
  searchPlace(){
    ...
    this.api.getAddress(this.streetAddress).subscribe(
      result => {
        console.log(result);
        this.addressList = [];
        var tempAddress = [];
        ...
        for (var i = 0; i < features.length; i++){
          tempAddress.push(
          features[i]['properties']['address_line1'] + " "+
          features[i]['properties']['address_line2'] + " "+
          features[i]['properties']['city']+ " "+
  ...
  searchResult(places: string){
    this.streetAddress = places;
    this.addressList = [];
  }
